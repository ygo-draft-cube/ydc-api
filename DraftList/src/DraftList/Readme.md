...
WORKDIR /src
COPY ["GetDraftList.csproj", "GetDraftList/"]
RUN dotnet restore "GetDraftList/GetDraftList.csproj"

WORKDIR "/src/GetDraftList"
COPY . .
RUN dotnet build "GetDraftList.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "GetDraftList.csproj" \
...
Execute unit tests
```
    cd "GetDraftList/test/GetDraftList.Tests"
    dotnet test
```

Deploy application
```
    cd "GetDraftList/src/GetDraftList"
